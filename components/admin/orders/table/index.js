...
function Row(props) {
  ...
  const handleSetCompleted = async (id) => {
    try {
      const { data } = await axios.put(`/api/admin/order/${id}/complete`, {
        id: session.user.id,
        status: "succeeded",
      });
  ...
  const handleSetCancelled = async (id) => {
    try {
      const { data } = await axios.put(`/api/admin/order/${id}/cancel`);
      if (data?.error) return toast.error(data.error);
      setOrders(data.orders);
      toast.success(data.message);
    } catch (error) {
  ...
  return (
    <React.Fragment>
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        ...
        <TableCell align="right">
          <RiCheckboxFill onClick={() => handleSetCompleted(row._id)} />
          <RiCheckboxIndeterminateFill
            onClick={() => handleSetCancelled(row._id)}
          />
        </TableCell>
      </TableRow>
      <TableRow>
...
export default function CollapsibleTable({ rows, setOrders }) {
  return (
    <TableContainer component={Paper}>
      ...
      <Table aria-label="collapsible table" className={styles.table}>
        ...
        <TableBody>
          {rows.map((row) => (
            <Row key={row.name} row={row} setOrders={setOrders} />
          ))}
        </TableBody>
      </Table>
    </TableContainer>
